...
const type = contract.resolveCollection('@keet/devices')
{
  ...
  restructure(version, keyBuffer, valueBuffer) -> { ...key, ...value }, (only on coll)
  indexes
}
...
function generateCollectionDefinition (id, collection) {
  ...
  str += `  restructure: ${id}_reconstruct,\n`
  str += `  indexes: []\n`
  str += '}\n'
  return str
}
...
function generateEncodeKeyRange (id, index) {
  ...
  str += `      gt: gt ? indexify_${id}(gt) : null,\n`
  str += `      lt: lt ? indexify_${id}(lt) : null,\n`
  str += `      gte: gte ? indexify_${id}(gte) : null,\n`
  str += `      lte: lte ? indexify_${id}(lte) : null,\n`
  str += '    })\n'
  str += `  }`
  return str
}
...
function generateRestructure (collection) {
  let str = ''
  return str
}
