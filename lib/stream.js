...
module.exports = class IndexStream extends Readable {
  constructor (stream, { overlay = [], reverse = false, asap = false, map = null }) {
    super()
  ...
  async _mapAndProcess () {
    ...
    while (!Readable.isBackpressured(this) && this.destroying === false) {
      ...
      if (promises.length === 0 && isEnded(this.stream) === false) break

      try {
        if (this.asap === true && promises.length > 1) {
          await this._processAsap(promises)
        } else {
          const batch = await Promise.all(promises)
          if (this.destroying === false) this._process(batch)
        }
      } catch (err) {
