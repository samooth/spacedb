...
module.exports = class IndexStream extends Readable {
  constructor (db, range, { index = null, collection = index.collection, reverse = false, limit = -1, overlay = [] }) {
    ...
    this.index = index
    this.stream = this.engine.createReadStream(this.snapshot, range, { reverse, limit })
    ...
    if (this.snapshot !== null) this.snapshot.ref()
  }
  ...
  _continueDestroyMaybe () {
    ...
    this.continueDestroy = null
    if (this.snapshot !== null) this.snapshot.unref()
    cb(null)
  }
  ...
  async _mapAndProcess () {
    ...
    while (!Readable.isBackpressured(this) && this.destroying === false) {
      ...
      const promises = entries.length === 0 ? entries : this.engine.getIndirectRange(this.snapshot, this.index.reconstruct, entries)
