...
module.exports = class IndexStream extends Readable {
  constructor (stream, { restructure = null, overlay = [], limit = -1, reverse = false, asap = false, map = null }) {
    super()
  ...
  _push (data) {
    ...
    this.push(this.restructure === null ? data : this.restructure(data.key, data.value))
  }
  ...
  _drain () {
    ...
    if (this.map === null) {
      this._process(fullyDrain(this.stream), isEnded(this.stream))
